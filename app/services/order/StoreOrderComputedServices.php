...
class StoreOrderComputedServices extends BaseServices
{
    ...
    public function computedOrder(int $uid, string $key, array $cartGroup, int $addressId, string $payType, bool $useIntegral = false, int $couponId = 0, bool $isCreate = false, int $shippingType = 1,$electronic_code='')
    {
        ...
        if (!array_key_exists($payType, $this->payType)) {
    ...
    public function useCouponId(int $couponId, int $uid, $cartInfo, $payPrice, bool $isCreate)
    {
        ...
        if ($couponId) {
            ...
            $couponInfo = $couponServices->getOne(['id' => $couponId, 'uid' => $uid, 'is_fail' => 0, 'status' => 0]);
            if (!$couponInfo) {
                throw new ValidateException('选择的优惠劵无效!');
            }
            $coupons = $couponServices->getUsableCouponList($uid, ['valid' => $cartInfo], $payPrice);
            $flag = false;
    ...
    public function useIntegral(bool $useIntegral, $userInfo, string $payPrice, array $other)
    {
        ...
        if ($useIntegral && $userInfo['integral'] > 0) {
            ...
            if ($deductionPrice < $payPrice) {
                ...
                $usedIntegral = (float)bcdiv((string)$payPrice, (string)$other['integralRatio'], 2);
                $SurplusIntegral = bcsub((string)$userInfo['integral'], (string)$usedIntegral, 2);
                $payPrice = 0;
            }
        } else {
            $deductionPrice = 0;
